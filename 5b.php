...
foreach($input as $line){
    ...
    if($x1 === $x2){
        ...
        $map = processDiagonal($map, $x1, $x2, $y1, $y2);
        continue;
    }else{
        continue;
    }
...
function processDiagonal(array $map, $x1, $x2, $y1, $y2):array
{
    $xDiff = $x1 < $x2 ? 1 : -1;
    $yDiff = $y1 < $y2 ? 1 : -1;
    ...
    $y = $y1;
    while($x !== $x2 + $xDiff) {
        $map[$y][$x] = !isset($map[$y][$x]) ? 1 : $map[$y][$x] += 1;
        $x+=$xDiff;
        $y+=$yDiff;
    }
...
function recursiveArrayCount(array $array):int{
    $count = 0;
